...
# deepTools packages
import cfDNA_GCcorrection.utilities
from cfDNA_GCcorrection import bamHandler
from cfDNA_GCcorrection import mapReduce
...
class CountReadsPerBin(object):
    ...
    def __init__(self, bamFilesList, binLength=50, numberOfSamples=None, numberOfProcessors=1,
        ...
        if extendReads and len(bamFilesList):
            from cfDNA_GCcorrection.getFragmentAndReadSize import get_read_and_fragment_length
    ...
    def run(self, allArgs=None):
        ...
        chromsizes, non_common = cfDNA_GCcorrection.utilities.getCommonChrNames(bamFilesHandles, verbose=self.verbose)
        ...
        # Handle GTF options
        transcriptID, exonID, transcript_id_designator, keepExons = cfDNA_GCcorrection.utilities.gtfOptions(allArgs)
    ...
    def count_reads_in_region(self, chrom, start, end, bed_regions_list=None):
        ...
        if self.save_data:
            _file = open(cfDNA_GCcorrection.utilities.getTempFileName(suffix='.bed'), 'w+t')
    ...
    def get_coverage_of_region(self, bamHandle, chrom, regions,
        ...
        for idx, reg in enumerate(regions):
            ...
            for read in bamHandle.fetch(chrom, regStart, regEnd):
                ...
                # Fragment lengths
                tLen = cfDNA_GCcorrection.utilities.getTLen(read)
